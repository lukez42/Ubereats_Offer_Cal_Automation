...
(function () {
    ...
    function extractDateFromDrawer(drawer) {
        ...
        for (const el of dateParagraphs) {
            ...
            // Match patterns like "Nov 13, 2025" or "13/11/2025"
            if (/\w{3}\s+\d{1,2},\s+\d{4}|\d{1,2}\/\d{1,2}\/\d{4}/.test(text)) {
                return text;
            }
        }
        return "";
        ...
        Swal.fire({
            ...
            html: `
                <div style="text-align: center; margin-bottom: 20px;">
                    <h2 style="margin: 0; color: #DE1135;">£${totalOfferSum.toFixed(2)}</h2>
                    <p style="margin: 5px 0;">Total Offer Sum</p>
                    <p style="margin: 5px 0; color: #666;">Found in <strong>${ordersWithOffers}</strong> out of <strong>${finalProcessedCount}</strong> processed orders.</p>
